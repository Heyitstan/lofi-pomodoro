import {gifList} from './giphy'

var generateNewBackground = () => {
  const rndInt = Math.floor(Math.random()*14);
  const newGif = gifList[rndInt]
  newBackground(newGif)
};

var newBackground = function (newGif) {
  var myDiv = document.getElementById('bg'); // grabs #bg
  var oldImg = document.getElementById('background');
  oldImg.remove();
  var myPath = newGif; // initialises string variable myPath
...
export default generateNewBackground